...
export default function EnterpriseRegisterPage() {
  ...
  const phuongDKKDOptions = useMemo(
    () => WARDS_BY_PROVINCE[form.tinh] ?? [],
    [form.tinh],
  );
  const phuongHDOptions = useMemo(
    () => WARDS_BY_PROVINCE[form.tinhHD] ?? [],
    [form.tinhHD],
  );
  ...
  const validateAndOtp = async () => {
    ...
    try {
      ...
      if (emailCheck.exists)
        preErrors.email = "Email đã được sử dụng cho doanh nghiệp khác";
  ...
  const confirmOtp = async () => {
    ...
    try {
      ...
      setToast({
        message: e instanceof Error ? e.message : "Xác thực OTP thất bại",
        variant: "error",
      });
    } finally {
  ...
  const confirmWizard = async () => {
    ...
    try {
      ...
      if (form.diaDiem.trim())
        fd.append("operatingAddress", form.diaDiem.trim());
      if (form.nguoiDD.trim()) fd.append("representative", form.nguoiDD.trim());
      if (form.sdtDD.trim())
        fd.append("representativePhone", form.sdtDD.trim());
      ...
      const message =
        e instanceof Error ? e.message : "Đã có lỗi xảy ra. Vui lòng thử lại.";
  ...
  const reviewRows: [string, string][] = [
    ...
    [
      ...
      [form.phuong, form.tinh].filter(Boolean).join(", "),
    ],
      ...
      <div className="mx-auto w-[940px] max-w-[96vw] rounded-xl bg-white shadow-[0_15px_45px_rgba(0,0,0,0.06)]">
        ...
        <div className="flex items-center justify-center gap-0 pb-3 pt-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-[13px] ${wizardStep === 2 ? "font-semibold text-ink" : "font-medium text-[#9ca3af]"}`}
            >
              Xác nhận đăng ký
            </span>
          </div>
            ...
            <div className="px-7 pb-6">
                  ...
                  <OutlinedTextField
                    ...
                    onChange={(val) => {
                      ...
                      if (wizardFieldErrors.ten)
                        setWizardFieldErrors((p) => ({ ...p, ten: undefined }));
                    }}
                ...
                <div className="mb-6 grid grid-cols-3 gap-3.5">
                  ...
                  <OutlinedTextField
                    ...
                    onChange={(val) => {
                      ...
                      if (wizardFieldErrors.sdt)
                        setWizardFieldErrors((p) => ({ ...p, sdt: undefined }));
                    }}
              ...
              <div className="overflow-hidden rounded-lg border border-[#e5e7eb]">
                <table className="w-full border-collapse text-[13px]">
                  <thead>
                    <tr>
                      <th className="w-50 border-b border-[#e5e7eb] bg-[#f9fafb] px-3 py-2 text-left text-[12.5px] text-[#374151]">
                        Tên file
                      </th>
                  ...
                  <tbody>
                    {FILE_NAMES.map((name, idx) => (
                      <tr
                        key={name}
                        className="border-b border-body last:border-b-0"
                      >
            ...
            <div className="px-7 pb-6">
              ...
              <div className="rounded-lg border border-[#e5e7eb] p-5">
                {reviewRows.map(([label, value]) => (
                  <div
                    key={label}
                    className="flex border-b border-body py-2.5 last:border-b-0"
                  >
                    <span className="w-70 shrink-0 text-[13.5px] font-semibold text-[#374151]">
                      {label}
                    </span>
